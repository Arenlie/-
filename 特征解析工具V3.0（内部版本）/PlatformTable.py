...
from excel_Optimization import export_to_excel
...
def output_template(parm_data, bearing_data):
    ...
    ylb_SWE, ylb_SWPE, ylb_SWPA, ylb_vel_rms, ylb_kur, ylb_acc_rms, ylb_impulse, \
        ...
        Voltage_Pos_THD, Voltage_Neg_THD, Voltage_Zero_THD, Voltage_Total_THD, \
        = ['null'] * 146

    eq_name, eq_code, point_name, point_code, sensor_type, DW_type, L, N, nc, n, f0, m, Bearing_designation, \
        Manufacturer, Z, vane, G_vane, EDF1, EDF2, fc1, fb1, fc2, fb2, F_min1, F_max1, F_min2, F_max2 = parm_data
    # 中间计算值
    PPF = "/"
    if point_code[-2:-1] not in ['X', 'Y', 'Z']:
        if sensor_type == '应力波':
...
def output_template_all(excel_path, my_deftable, output_path):
    """
    :param my_def: 特征对应注释
    ...
    desired_columns = ['设备名称', '设备编码', '测点名称', '测点编码', '传感器类型', '网关型号',
    ...
    for df_index, df_row in input_data.iterrows():
        eq_name, eq_code, point_name, point_code, sensor_type = df_row.iloc[:5]
        ...
        if sensor_type == '应力波':
            tmp_data.append(
                [eq_name, eq_code, point_name, point_code, sensor_type, '偏置电压',
                 str(point_code) + '999', '应力波特征', '模拟量', 'V'])
        elif sensor_type == "加速度" and point_code[-2:-1] not in ['X', 'Y', 'Z']:
            tmp_data.append(
                [eq_name, eq_code, point_name, point_code, sensor_type, '偏置电压',
                 str(point_code) + '999', '时域特征', '模拟量', 'V'])
        elif sensor_type == "速度":
            tmp_data.append(
                [eq_name, eq_code, point_name, point_code, sensor_type, '偏置电压',
                 str(point_code) + '999', '时域特征', '模拟量', 'V'])
            ...
            tmp_data.append(
                [eq_name, eq_code, point_name, point_code, sensor_type, '轴承温度',
                 str(point_code) + '000', '温度', '模拟量', '℃'])
        elif sensor_type == "转速":
            tmp_data.append(
                [eq_name, eq_code, point_name, point_code, sensor_type, '转速',
                 str(point_code) + '777', '工艺', '模拟量', 'rpm'])
        elif sensor_type == "电流谱":
            tmp_data.append(
                [eq_name, eq_code, point_name, point_code, sensor_type, '偏置电压',
                 str(point_code) + '999', '时域特征', '模拟量', 'V'])
...
if __name__ == "__main__":
    output_template_all(r"H:\chaos项目资料\特征解析工具汇编\测试文件\data_all(模板).xlsx",
                        ...
                        r"H:\chaos项目资料\特征解析工具汇编\测试文件\data_all - 平台导入表(电流电压).xlsx")
